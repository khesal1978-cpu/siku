...
export default function StatsCard({ icon: Icon, label, value, subtext, variant = 'default' }: StatsCardProps) {
  return (
    ...
    >
      <Card
        className={`p-4 ${variant === 'highlight' ? 'bg-primary/5 border-primary/20' : ''}`}
        data-testid={`card-stat-${label.toLowerCase().replace(/\s+/g, '-')}`}
      >
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg ${variant === 'highlight' ? 'bg-primary/10' : 'bg-muted'}`}>
          ...
          <div className="flex-1">
            ...
            <p className="text-lg font-bold font-['Poppins'] tabular-nums" data-testid="text-stat-value">{value}</p>
            {subtext && (
